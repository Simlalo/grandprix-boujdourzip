...
export default function PublicResults() {
  ...
  const [eventName, setEventName] = useState('الجائزة الكبرى للعدو الريفي');
  ...
  async function loadAll() {
    const { data: settings } = await supabase
      .from('settings')
      .select('key, value')
      .in('key', ['event_name', 'event_edition']);

    if (settings) {
      const name = settings.find(s => s.key === 'event_name')?.value;
      const edition = settings.find(s => s.key === 'event_edition')?.value;
      if (name) setEventName(`${name}${edition ? ' — ' + edition : ''}`);
    }
    ...
    setInstitutionStandings(Object.values(instMap).sort((a, b) => b.points - a.points));
      ...
      ['male', 'female'].forEach(gender => {
        ...
        qualResults.forEach(r => {
          ...
          if (!athleteMap[aid]) athleteMap[aid] = {
            id: aid, name: `${r.athlete.first_name} ${r.athlete.last_name}`,
            dossard: r.athlete.dossard_number, institution: r.athlete.institution?.name,
            qualPoints: 0, finPoints: 0,
          };
  ...
  const hasResults = institutionStandings.length > 0;
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f8fafc' }}>
      <div style={{
        ...
        <div style={{ fontSize: 13, opacity: 0.8, marginBottom: 4 }}>🏃 النتائج المباشرة</div>
        <div style={{ fontSize: 17, fontWeight: 900 }}>{eventName}</div>
        {lastUpdated && (
          <div style={{ fontSize: 11, opacity: 0.6, marginTop: 6 }}>
            آخر تحديث: {lastUpdated.toLocaleTimeString('ar')} • يتجدد كل 30 ثانية
          </div>
        )}
      </div>
      ...
      <div className="container" style={{ paddingTop: 16 }}>
        {!hasResults ? (
          <div className="card text-center" style={{ padding: 40, marginTop: 16 }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700 }}>لم تبدأ النتائج بعد</div>
            <div style={{ fontSize: 13, color: 'var(--text-muted)', marginTop: 8 }}>
              ستظهر النتائج هنا فور إدخالها من اللجنة
            </div>
          </div>
        ) : (
          <>
            ...
            {tab === 'standings' && (
              <PublicStandings standings={institutionStandings} />
            )}
            {tab === 'categories' && (
              <PublicCategories
  ...
  return (
    <div>
      <div className="card mb-4" style={{
        ...
        <div style={{ fontSize: 13, fontWeight: 700, opacity: 0.85, marginBottom: 4 }}>الجائزة الكبرى</div>
        <div style={{ fontSize: 26, fontWeight: 900, marginBottom: 6 }}>{winner.name}</div>
        <div style={{ fontSize: 20, fontWeight: 900, opacity: 0.9 }}>{winner.points} نقطة</div>
      </div>
      <div className="list">
        {standings.map((inst, idx) => (
          <div key={inst.id} className="card" style={{
            ...
            <div style={{ fontSize: 22, fontWeight: 900, color: idx === 0 ? '#92400e' : 'var(--primary)' }}>
              {inst.points}
            </div>
          </div>
        ))}
...
function PublicCategories({ categoryResults, selected, onSelect }) {
        ...
        return (
          <div key={key} className="card" onClick={() => onSelect(key)}
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              ...
              <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>{cat.athletes.length} رياضي ‹</div>
            </div>
...
function PublicCategoryDetail({ data, onBack }) {
  return (
    <div>
      <button onClick={onBack} className="btn btn-outline mb-4" style={{ fontSize: 14, minHeight: 48 }}>
        → الرجوع
      </button>
      <div className="card mb-4" style={{ background: 'var(--primary)', color: 'white', textAlign: 'center', padding: 16 }}>
        <div style={{ fontSize: 22, fontWeight: 900 }}>{data.label}</div>
        <div style={{ fontSize: 13, opacity: 0.8, marginTop: 4 }}>{data.athletes.length} رياضي</div>
      </div>
      <div style={{
        display: 'grid', gridTemplateColumns: '40px 1fr 60px 60px 70px',
        gap: 4, padding: '6px 12px', fontSize: 11, fontWeight: 700,
        ...
        {data.athletes.map((a, idx) => {
          ...
          return (
            <div key={a.id} style={{
              ...
              borderBottom: '1px solid var(--border)', alignItems: 'center', textAlign: 'center',
            }}>
              ...
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontSize: 14, fontWeight: 700 }}>
                  <span style={{ color: 'var(--accent)', fontWeight: 900 }}>#{a.dossard}</span> {a.name}
                </div>
